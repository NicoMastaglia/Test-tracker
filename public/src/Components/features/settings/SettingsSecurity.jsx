...
import  {PswValidator} from './PasswordRequirements';
// per la gestione dei campi password con visibilità toggle
    ...
    <div className="flex flex-col gap-1.5 w-full">
      ...
      <div className="relative w-full">
        <input
          ...
          className="w-full rounded-lg border border-slate-200 pl-3 pr-10 py-2.5 text-sm text-slate-800 transition-all focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 bg-white placeholder-slate-400"
        />
        <button
          type="button"
          onClick={() => setShowPassword(!showPassword)}
          className="absolute right-3 top-1/2 -translate-y-1/2 flex items-center text-slate-400 hover:text-slate-600 focus:outline-none"
        >
...
const SettingsSecurity = ({ handleUpdatePassword, handleChange, securityData = {}, isFirstLogin = false }) => {
 ...
 const validation = useMemo(() => {
  ...
  return (
    <div className="bg-white border border-slate-100 shadow-sm rounded-2xl flex flex-col justify-between w-full max-w-xl min-h-[500px]">
      <form onSubmit={onSubmit} className="flex flex-col grow justify-between">
        ...
        <div className="p-4 bg-slate-50/50 border-t border-slate-100 rounded-b-2xl flex justify-end mt-6">
          <button
            type="submit"
            disabled={isInvalid}
            className="flex items-center gap-2 bg-emerald-600 hover:bg-emerald-700 text-white font-semibold px-4 py-2 rounded-lg text-xs transition-all shadow-sm active:scale-[0.98] disabled:bg-slate-200 disabled:text-slate-400 disabled:scale-100 disabled:cursor-not-allowed disabled:shadow-none"
          >
            <KeyRound size={14} />
