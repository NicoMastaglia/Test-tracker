        ...
        {projectAssignedUsers.length > 0 ? (
          projectAssignedUsers.map((assignedUser, index) => {
            ...
            const assignedUserEmail =
              assignedUser.email ?? `ID ${assignedUserId}`;

            return (
              <div
                key={assignedUserId}
                className="mx-auto flex w-full max-w-2xl items-center justify-between gap-3 rounded-xl border border-slate-200 bg-white p-3 transition-colors hover:bg-slate-50"
              >
