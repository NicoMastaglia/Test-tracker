...
router.post("/:templateId/item", checkAdmin, async (req, res) => {
  ...
  try {
    ...
    "SELECT ct.id, ct.project_id FROM checklist_template ct JOIN project p ON ct.project_id = p.id WHERE ct.id = ? AND (p.created_by = ? OR p.manager_id = ? OR ?)",
...
router.put("/item/:id", checkAdmin, async (req, res) => {
  ...
  try {
    ...
    const [isOwnerResults] = await db.execute(
      "SELECT ci.id, ct.project_id FROM checklist_item ci JOIN checklist_template ct ON ci.template_id = ct.id JOIN project p ON ct.project_id = p.id WHERE ci.id = ? AND (p.created_by = ? OR p.manager_id = ? OR ?)",
...
router.delete("/item/:id", checkAdmin, async (req, res) => {
  ...
  "SELECT ci.id, ct.id AS template_id,p.id as project_id FROM checklist_item ci JOIN checklist_template ct ON ci.template_id = ct.id JOIN project p ON ct.project_id = p.id WHERE ci.id = ? AND (p.created_by = ? OR p.manager_id = ? OR ?)",
